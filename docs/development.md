...
Commands in this guide run from the repository root. For the research question and findings, see the [project overview](../README.md).
...
## Structured output for new collections

New `atlas create` and `scripts/run_languages.py prepare` commands default to
`great-circle-json-object-v3`. The Google request sets `responseMimeType` to
`application/json` and supplies `responseJsonSchema` with one required numeric
`distance_km` field and no additional properties. The schema and exact translated
prompt are stored in the experiment manifest. No minimum or maximum distance is
supplied to the model through the schema.

The parser decodes JSON and rejects strings, booleans, extra or duplicate keys,
non-finite values, and malformed documents. It does not use regular expressions
or coerce numeric strings. Geographic plausibility checks remain a separate step.
The schema changes the elicitation condition, so its observations form separate
comparison cohorts from the historical text-response studies below. Resuming an
existing run always uses its recorded protocol. Library callers must pass the
protocol explicitly; the low-level factory retains its legacy compatibility default.

```sh
uv run atlas create --dataset data/capitals-v1.csv --model gemini-3.5-flash --protocol great-circle-json-object-v3 --samples 1 --budget 1
# Or prepare a new multilingual schema condition:
uv run python scripts/run_languages.py prepare --group runs/languages-json-v3.json --protocol great-circle-json-object-v3 --languages en ar zh
```

A live integration check on 9 September 2026 queried all three pairs among London, Cairo, and Beijing once in English, Arabic, and Chinese: nine calls, nine schema-valid responses. Reported token usage implied $0.0017235 in estimated cost. This check verifies API integration; it is not included in the research comparisons. Automated tests also inspect the outgoing schema fields, reject ambiguous JSON, preserve raw responses across resume, and ensure the JSON parser never calls regular-expression parsing.
...
```sh
uv run python -m scripts.run_languages prepare --protocol great-circle-formatted-number-v2
...
× 3 languages = **148,500 collected responses**. French and Spanish
remain available in the historical 50-capital cohort.

```sh
uv run python scripts/run_languages.py prepare --protocol great-circle-formatted-number-v2 --group runs/languages-100-v3.json --dataset data/capitals-100-v3.csv --languages en ar zh --budget 15
...
The $15 cap is **per language**. The completed collection recorded $23.570781 in token-based estimated cost across the three runs; this is not a provider invoice. The conservative output-ceiling estimate is higher. No run may spend past its own recorded reservation ceiling.
...
and independent calls. Its null is full-distribution exchangeability, not equality of population median maps: unequal variability can also cause rejection. It does not infer coastline shape or a literal neural map.
