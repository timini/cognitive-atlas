...
interface MapAudit {export_ids:string[];total_pairs:number;permutations:number;holm_family_size:number;report_url:string;comparisons:{a:string;b:string;map_p_holm:number;null_mean_capital_shift_km:number;null_shift_95pct_interval_km:number[]}[]}
...
export default function LanguageEvidence({models,pairwise,selected,aggregation}:{models:Model[];pairwise:MapComparison[];selected:string;aggregation:Aggregation}) {
 ...
 const [mapAudits,setMapAudits]=useState<MapAudit[]>([]);
 const [choice,setChoice]=useState('ar');
 useEffect(()=>{const c=new AbortController();fetch(publicAsset('/data/language-inference.json'),{signal:c.signal}).then(r=>r.ok?r.json() as Promise<{cohorts:Audit[];map_cohorts?:MapAudit[]}>:null).then(r=>{if(r){setAudits(r.cohorts);setMapAudits(r.map_cohorts??[])}}).catch(()=>{});return()=>c.abort()},[]);
 ...
 const mapAudit=mapAudits.find(a=>a.export_ids.includes(baseline.export_id)&&a.export_ids.includes(target.export_id));
 const mapTest=mapAudit?.comparisons.find(c=>[c.a,c.b].includes('en')&&[c.a,c.b].includes(target.language));
 ...
 return <div className="language-evidence">
  ...
  <div className="evidence-cards">
   <div><span>Average capital shift</span><strong>{fmt(map?.mean_aligned_map_displacement_km)} <small>km</small></strong><p>After globally aligning the two spherical fits. A shift of tens of kilometres is subtle on a world map.</p>{mapTest&&aggregation==='median'&&<p className="map-test">Map-displacement randomization test: adjusted p = {fmt(mapTest.map_p_holm,4)}. Randomized-language maps shift by {fmt(mapTest.null_mean_capital_shift_km)} km on average.</p>}</div>
   <div><span>Difference in distance judgments</span><strong>{fmt(test?.mean_absolute_median_disagreement_km)} <small>km</small></strong><p>{test?`Mean absolute difference between pair medians. Adjusted p = ${fmt(test.judgment_p_holm,4)}${test.judgment_p_holm<.05?': detectable under the within-pair exchangeability test.':': no detectable departure under this test.'}`:'A matched inferential audit is not available in this view.'}</p></div>
  ...
  {mapAudit&&<p className="small-note">The separate direct map test refits both median-based spherical maps for each of {fmt(mapAudit.permutations)} within-pair label permutations, then measures their globally aligned capital displacement. It uses all {fmt(mapAudit.total_pairs)} pairs, preserves each language’s valid-answer count, and corrects {mapAudit.holm_family_size} map comparisons. This tests exchangeability of valid response distributions for these fixed capitals. Differences in variability can also produce rejection; this does not establish different underlying median maps. Coastline shapes are not tested. <a href={publicAsset(mapAudit.report_url)} download>Download map-shape audit and null statistics</a>.</p>}
  {audit&&<p className="small-note">Tests use {fmt(audit.complete_pairs)} common complete pairs of {fmt(audit.total_pairs)}, {fmt(audit.permutations)} within-pair permutations, and Holm correction across {audit.holm_family_size} tests. P-values concern response distributions for these fixed capitals, translations and model run, assuming independent calls. These distance-answer tests do not test map shape or establish a causal language effect. The MAE test is not a general test of equal average accuracy under otherwise different distributions. Tests always use medians; map shifts follow the selected aggregation. <a href={publicAsset(audit.report_url)} download>Download full statistical audit</a>.</p>}
 </div>;
}
